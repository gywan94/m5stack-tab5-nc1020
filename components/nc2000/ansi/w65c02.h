...
/* Tab5 fast read path: for normal memory (addr>=0x80) with no NOR command in
 * progress, a read is exactly memmap[addr>>13][addr&0x1FFF] — Load()'s
 * dummy_io_for_read is a no-op stub here, read_nor only intercepts mid-command,
 * and IO/zp40 live at addr<0x80. That covers ~all opcode/operand/RAM/stack
 * reads, so inlining it avoids the CLoad()+read_nor()+branch-chain that is the
 * real emulation bottleneck. Everything else falls back to the full Load(). */
extern uint8_t* memmap[8];
extern uint8_t* g_fp_type_ptr;
#ifndef NC2K_FAST_MEM
#define NC2K_FAST_MEM 1   /* set 0 (via -DNC2K_FAST_MEM=0) to A/B-test the fast paths */
#endif
inline uint8_t CPU_PEEK(uint16_t addr){
#if NC2K_FAST_MEM
  if(addr >= 0x80 && (!g_fp_type_ptr || *g_fp_type_ptr == 0)){
    return memmap[unsigned(addr) >> 13][addr & 0x1FFF];
  }
#endif
...
{
  /* Tab5 fast write path: 0x80..0x3FFF is always plain RAM (zero page, the 6502
   * stack at 0x100-0x1FF, and low RAM) — never IO/zp40 (addr<0x80) and never a
   * NOR bank (those map at 0x4000+), so a direct store matches Store() exactly
   * and skips dummy_io_for_write()+write_nor()+branches on the hottest writes. */
#if NC2K_FAST_MEM
  if(addr >= 0x80 && addr < 0x4000){
    memmap[unsigned(addr) >> 13][addr & 0x1FFF] = a;
    return;
  }
#endif
